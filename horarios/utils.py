...
def _importar_ics(url, usuario):
    ...
    eventos_raw = []  # Acumulamos antes de guardar
    ...
    # Fusionar eventos consecutivos del mismo ramo en el mismo día
    eventos_raw.sort(key=lambda e: (e['fecha'], e['ramo'], e['hora_inicio']))
    eventos_fusionados = []

    for evento in eventos_raw:
        if eventos_fusionados:
            ultimo = eventos_fusionados[-1]
            mismo_ramo = ultimo['ramo'] == evento['ramo']
